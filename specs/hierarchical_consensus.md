...
## Architecture
The system starts with a _rootnet_ that, at first, keeps the entire state and processes all the transactions in the system (like present-day Filecoin). Any subset of users of the rootnet can spawn a new subnet from it.
...
From the perspective of a peer in the network, spawning or syncing with a new subnet starts a set of new processes to handle the independent state, mempool, and specific [GossipSub](https://github.com/libp2p/specs/blob/master/pubsub/gossipsub/gossipsub-v1.1.md) topic to broadcast and receive subnet-specific messages.
